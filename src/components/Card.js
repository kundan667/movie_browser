    ...
    return (
        <div className="card bg-base-100 shadow-xl rounded-lg overflow-hidden relative h-full w-full">
            <figure className='w-full h-full'>
                <img src={imageUrl} loading="lazy" alt={data.title} className="w-full h-full object-cover" onError={(e) => handleError(e)} />
            </figure>
            <div className="card-body w-full absolute bottom-0 left-0 text-left p-3 text-gray-100 bg-card-body-gradient pt-[3rem] block leading-none">
                <div className='flex items-center justify-between pb-1'>
                    <h2 className="card-title font-semibold text-base sm:text-xl overflow-hidden block whitespace-nowrap truncate pr-[1rem] w-[80%]" title={data.title}>{data.title}</h2>
                    <div className='flex items-center justify-center w-[20%]'>
                        <img src="/assets/common/star.png" loading="lazy" alt="star" width={13} className='mr-1 w-[10px] sm:w-[13px]' />
                        <span className='font-bold text-sm sm:text-base'>{data.vote_average.toFixed(1)}</span>
                    </div>
                </div>
